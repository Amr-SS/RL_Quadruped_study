...
class GenesisVecEnv(VecEnv):
    ...
    def step_wait(self):
        ...
        # 4. Increment step counters
        self._step_counts += 1

        # 5. Auto-reset any robots that fell this step
        done_indices = np.where(dones_cpu)[0]
        if len(done_indices) > 0:
            reset_obs = self._do_reset(done_indices)
            obs_cpu[done_indices] = reset_obs[done_indices]

        self._last_obs = obs_cpu

        # SB3 expects a list of dicts for infos
