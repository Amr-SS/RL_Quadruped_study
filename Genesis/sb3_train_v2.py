...
def main():
    ...
    model = PPO(
        ...
        vf_coef=1.0,                    # match RSL-RL value_loss_coef
        max_grad_norm=1.0,              # match RSL-RL max_grad_norm
        gamma=0.99,
        gae_lambda=0.95,
        tensorboard_log="./push_training_logs/",
        policy_kwargs=dict(
            net_arch=dict(
                pi=[512, 256, 128],     # match RSL-RL actor network
                vf=[512, 256, 128],     # match RSL-RL critic network
            ),
            activation_fn=torch.nn.ELU, # match RSL-RL activation
        ),
    )
